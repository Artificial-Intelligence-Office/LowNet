...
# -*- coding: utf-8 -*-
"""
Created on Thu Jun  4 16:28:24 2020
...
def ckld_loss(y_pred,y_true, alpha=0.5,gamma=3,epsilon=1e-10):
...
def plot():
    ...
    plt.title('model accuracy_'+str(conf)+str(i))
    plt.ylabel('accuracy')
    plt.xlabel('epoch')
...
class Net(nn.Module):
    def __init__(self,slopes=[1,1,1]):
    ...
    def forward(self, x):
        #x = self.maxpool1(new_relu(self.conv1(x), slope=self.slopes[0]))
        x = new_relu(self.conv1(x), slope=self.slopes[0])

        x = self.maxpool2(new_relu(self.conv2(x), slope=self.slopes[1]))
        #x = new_relu(self.conv2(x), slope=self.slopes[1])

        x = self.maxpool3(new_relu(self.conv3(x), slope=self.slopes[2]))
...
def train(model, criterion, optimizer, num_epochs=250,alpha=0.5,beta=3):
    model.train()
        ...
        for phase in [ 'training','testing']:
            ...
            for inputs , labels in dataloaders[phase]:
                ...
                with torch.set_grad_enabled(phase == 'training'):
                    ...
                    loss = criterion(outputs, targets,alpha,beta)
...
path='Datas/poses_'+str(size)+'x'+str(size)+'_cleaned_reduced_splitted'
    ...
    for clas in classes:
        if True:
            ...
            for img in imgs :
                ...
                #data['y_'+k+'_'+str(fold)].append(class_index['New class'][int(clas)])
#                if int(clas)==11:
#                    data['y_'+k+'_'+str(fold)].append(21)
#                elif int(clas)==35:
#                    data['y_'+k+'_'+str(fold)].append(12)
#                elif int(clas)==32:
#                    data['y_'+k+'_'+str(fold)].append(34)
#                else:
...
dest_path='Results/Final/'+str(resize)+'/CKLD/'
dest_path_new=dest_path+'conf'+str(conf)+'/'
slope=[1,1,1]
alpha=0.5
beta=3
...
for i in range(3):
    ...
    model=train(model.cuda(),loss,optimizer,num_epochs=epochs,alpha=alpha,beta=beta)



    tail=str(conf)+'_'+str(i)
    
    torch.save(model.state_dict(), dest_path_new+'model_conf'+str(conf)+'_'+str(i)+'.pth')
